...
useEffect(() => {
  if (!userData) return;  // Don't start until userData is loaded
  ...
  const startTimeout = setTimeout(() => {
    if (isMounted && !isSpeakingRef.current && !isRecognizingRef.current && userData) {
      try {
        recognition.start();
        console.log("Recognition requested to start");
      } catch (e) {
  ...
  recognition.onresult = async (e) => {
    ...
    // Respond to the chosen name and the generic wake word "assistant".
    if (assistantName && (spokenText.includes(assistantName) || spokenText.includes("assistant"))) {
      setAiText("");
